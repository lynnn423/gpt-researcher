...
description: Audit a research report into an auditable deliverable — every claim labeled verified/uncertain/not found with confidence, source URL, source tier, negative findings documented, organized by sections with executive summary, English or Chinese (--lang). Pipeline: run_audit.py labels traceability; deep_check.py fact-checks load-bearing + weak claims, merging confirmed/false/misleading verdicts. This is the audit + fact-check layer of the audit-research system (research via the audit-research MCP deep_research, then audit with this skill). USE THIS when the user asks to audit a research report, make a deliverable auditable/traceable/every-claim-verified, prepare due-diligence or IC/LP material where each conclusion must be verifiable, run the audit layer, add confidence labels / source tiering / negative-evidence search paths, or run "audit-research". Do NOT use for the research itself (competitive-analysis/weizhena or the audit-research MCP) nor for standalone fact-checking (bullshit-detector).
---
